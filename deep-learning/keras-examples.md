| [file](./examples)             | Description                              | process | mark                     |
| :----------------------------- | :--------------------------------------- | ------- | ------------------------ |
| addition_rnn                   | An implementation of **sequence to sequence learning** for performing addition | 5.done  | 序列长度变化时如何处理              |
...
| cifar10_cnn                    | Train a simple deep **CNN** on the CIFAR10 small images dataset | done    |                          |
| conv_filter_visualization      | **Visualization** of the filters of VGG16, via gradient ascet in input space |         |                          |
| conv_lstm                      | This script demonstrates the use of a **convolutional** **LSTM** network | done    | ConvLSTM2D               |
...
| imdb_bidrectional_lstm         | Train a **Bidirectional LSTM** on the IMDB sentiment classification task | done    |                          |
| imdb_cnn                       | This example demonstrates the use of **Convolution1D** for text classification | done    |                          |
| imdb_cnn_lstm                  | Train a **recurrent convolutional network** on the IMDB sentiment classification task | done    |                          |
| imdb_fasttext                  | This example demonstrates the use of **fasttext** for text classification |         |                          |
| imdb_lstm                      | Trains a **LSTM** on the IMDB sentiment classification task | 3.done  |                          |
| lstm_benchmark                 | Compare **LSTM** implementations on the IMDB sentiment classification task | done    | RNN 3种计算方式的差异            |
| lstm_text_generation           | Example script to generate text from Nietzsche's writings. | 4.done  |                          |
| mnist_acgen                    | Train an Auxiliary Classifier Generative Adversarial Network (ACGAN) on the MNIST dataset |         |                          |
| mnist_cnn                      | Trains a **simple convnet** on the MNIST dataset | 2.done  |                          |
| mnist_hierarchical_rnn         | This is an example of using **Hierarchical RNN** (HRNN) to classify MNIST digits | done    |                          |
| mnist_irnn                     | This is a reproduction of the **IRNN** experiment with pixel-by-pixel sequential MNIST | done    | 初始化方式                    |
| mnist_mlp                      | Trains a **simple deep NN** on the MNIST dataset | 1.done  |                          |
...
| mnist_sklearn                  | Example of how to use sklearn wrapper    | done    | 用sklearn的GridSearch调试超参数 |
...
| reuters_mlp                    | Trains and evaluate a **simple MLP** o the Reuters newswire topic calssification task | done    |                          |
| stateful_lstm                  | Example script showing how to use **stateful RNNs** to model long sequences efficiently | done    |                          |
